...
actual fun ChoosePrinter(navController: NavController, bt: BluetoothHandler) {
    ...
    // This initial check can be removed since we know which library we are using

    // !! Location MUST be on and enabled, otherwise, Printooth will not be able to pull a list
    // of nearby devices
    // check which printer we are using
    if (bt.getConnection() == "Printooth") {
        println("We are doing the scan...")
        (bt as BluetoothHandlerImp).printoothBT.startPrinter(bt.getContext(), bt.getLauncher())
